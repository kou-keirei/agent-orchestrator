...
func TestEffectiveHarnessAndAgentConfig(t *testing.T) {
	...
	if got := effectiveAgentConfig(domain.KindWorker, cfg); got.Model != "worker" || got.Mode != "high" || got.Permissions != domain.PermissionModeAuto {
		t.Fatalf("worker config = %#v, want model=worker mode=high permissions=auto", got)
	}
	...
	for _, harness := range []domain.AgentHarness{domain.HarnessAider, domain.HarnessCodex} {
		got := effectiveAgentConfig(domain.KindWorker, unpinned)
...
func TestResolveChatAgentConfigPreservesEffortPresence(t *testing.T) {
	m := &Manager{}
	...
	tests := []struct {
		...
		{name: "omitted inherits project effort", want: "high"},
		{name: "explicit empty selects provider default", effortOverride: true},
		{name: "explicit value forwards exact effort", effort: "low", effortOverride: true, want: "low"},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			resolved, err := m.resolveChatAgentConfig(context.Background(), ports.SpawnConfig{
				ProjectID: "p", Kind: domain.KindWorker, Harness: domain.HarnessCodex,
				AgentConfig: ports.AgentConfig{Effort: test.effort}, EffortOverride: test.effortOverride,
			}, project)
			if err != nil {
				t.Fatal(err)
			}
			if resolved.Effort != test.want {
				t.Fatalf("resolved effort = %q, want %q", resolved.Effort, test.want)
			}
		})
	}
}
